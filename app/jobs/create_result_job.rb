class CreateResultJob < ApplicationJob
  ...
  def perform(user)
    FileUtils.cp(user.solution_file.path, Rails.application.secrets.solution_directory + '/CountAs.java')
    FileUtils.cp(user.test_file.path, Rails.application.secrets.solution_directory + '/Cuboid.java')
